...
const ProductDetailPopup: React.FC<ProductDetailPopupProps> = ({
  ...
  // State để quản lý ảnh đang được hiển thị (dùng url)
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {product.ingredients.map(
                    (ing: string | string[], index: number) => {
                      // Nếu ingredient là chuỗi, tách theo dấu phẩy
                      const ingList =
                        typeof ing === "string" ? ing.split(",") : ing;
                      return ingList.map((singleIng: string, idx: number) => (
                        <div
                          key={`${index}-${idx}`}
